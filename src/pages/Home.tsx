...
const Home: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Blog Educacional FIAP</h1>
      ...
      {loading ? (
        ...
        <div className="grid gap-6">
          {posts.length > 0 ? (
            posts.map((post) => (
              <article key={post.id} className="p-6 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                <Link to={`/post/${post.id}`}>
                  <h2 className="text-2xl font-semibold text-gray-800 mb-2 hover:text-blue-600 transition-colors">
                    {post.title}
                  </h2>
                </Link>
                <p className="text-sm text-gray-500 mb-4">Por <span className="font-medium">{post.author}</span> • {new Date(post.createdAt).toLocaleDateString()}</p>
